...
async def test_echo_round_trips_arguments() -> None:
    ...
    result = await executor.execute(name="echo", arguments=arguments)
...
async def test_echo_defaults_tool_use_id_when_absent() -> None:
    executor = EchoToolExecutor()
    result = await executor.execute(name="echo", arguments={"query": "hi"})
...
async def test_echo_forced_error_returns_is_error_true() -> None:
    ...
    result = await executor.execute(
        name="echo", arguments={"tool_use_id": "tu_2", "__force_error__": True}
    )
...
async def test_echo_output_is_capped() -> None:
    executor = EchoToolExecutor()
    result = await executor.execute(name="echo", arguments={"payload": "x" * 5000})
...
async def test_echo_sleep_flag_delays_before_returning() -> None:
    ...
    await executor.execute(name="echo", arguments={"__sleep__": 0.05})
