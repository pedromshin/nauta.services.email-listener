...
| **D-58-01 — VISUAL IDENTITY LOCKED (USER-PICKED 2026-07-15): "Provenance × Meaningful Colour"** — Direction A (Provenance) is the base; Direction C contributes its colour *philosophy* only; both light and dark themes are first-class. Three laws: (1) **colour is earned, never decorative** — chrome is monochrome, hues appear only where they carry data meaning: verdigris = confirmed, pencil-amber = suggested, madder = irreversible (destructive only); every button/link/nav/selection/focus ring is ink. (2) **Chrome speaks sans, evidence speaks serif** — content from the user's own mail is serif, no exceptions. (3) **Entity type is shape, never hue** (square/circle/diamond/triangle/hollow) — this is what frees colour for tier. Signature: the provenance mark (OCR-polygon-derived highlight; solid = confirmed, dashed = suggested). Full contract + oklch ladder + measured contrast: `phases/58-visual-identity-sketch-pick-human-gate/58-IDENTITY.md`. Reference render: `sketches/direction-final.html` | The user reviewed three distinct directions rendered on REAL polytoken screens (not swatches/mood boards) and picked, verbatim: *"i liked a best but also liked c color concept. we will want light and dark theme and liked the idea of using colors meaningfully datafully"*. This is 999.18(d) — the design-review loop on real screens that v1.9's autonomous approach skipped, which is exactly what produced "the whole UI is still ugly/experimental." Nine milestones of capability, zero of design; measurement enforced hygiene and hygiene passed every time while the product looked unfinished. Colour-as-data is the answer because polytoken's substance IS the tier ladder | — Pending. LOCKED input for Phases 59-63 — not re-litigated downstream without a new user decision. Two of Direction A's original values (`--sugg`, `--pencil`) failed WCAG-AA and are corrected in the locked ladder — found only by computing ratios, not eyeballing. One open item flagged at lock: **entity-type-as-shape was inferred from the user's colour law, not instructed** — cheap to revisit in Phase 59, expensive after Phase 62. Light tier-on-wash contrast has only ~0.09 headroom (4.59 vs 4.5) — any lightness drift breaks the gate |
